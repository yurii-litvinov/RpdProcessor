...
module Views =
    ...
    let frequentPhrases () =
        ...
        div [] [
            ...
            h2 [] [encodedText "3.3.1 Характеристики аудиторий (помещений, мест) для проведения занятий"]
            ...
            h2 [] [encodedText "3.3.2 Характеристики аудиторного оборудования, в том числе неспециализированного компьютерного оборудования и программного обеспечения общего пользования"]

            +("В аудиториях, где проводятся лекционные занятия, необходимо наличие досок и средств письма на них. Для показа слайдов необходим компьютер с "
             + "установленным программным обеспечением для работы со слайдами в форматах PDF, PPT, PPTX и подключенный к нему мультимедийный проектор с экраном.")

            h2 [] [encodedText "3.3.5 Перечень и объёмы требуемых расходных материалов"]
            +("Для аудиторий с маркерными досками необходимы стирающиеся маркеры в объёме, достаточном для проведения курса. Для аудиторий с меловыми досками "
             + "необходим мел в объёме, достаточном для проведения курса. Канцелярские принадлежности в объёме, достаточном для проведения курса.")
        ]
